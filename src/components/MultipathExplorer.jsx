  ...
  return (
    <div className="multipath-lab">
      ...
      <div className="multipath-readout">
        <div><small>MAX EXCESS DELAY</small><strong>{delayUs.toFixed(1)} µs</strong><span>latest path - first path</span></div>
        <div><small>MEAN DELAY</small><strong>{meanDelay.toFixed(2)} µs</strong><span>power-weighted</span></div>
        <div><small>RMS DELAY SPREAD</small><strong>{rmsDelay.toFixed(2)} µs</strong><span>power-weighted spread</span></div>
      ...
      <div className="multipath-grid">
        <div className="impulse-response-panel">
          <div className="visual-caption"><span>CHANNEL IMPULSE RESPONSE</span><strong>h(τ)</strong></div>
          <svg viewBox="0 0 420 240" role="img" aria-label="Two-tap multipath channel impulse response">
            ...
            <text className="mp-label" x={82 + delayUs * 12} y="215">{delayUs.toFixed(1)} µs</text>
