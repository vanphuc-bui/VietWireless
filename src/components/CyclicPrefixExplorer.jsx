  ...
  return (
    <div className="cp-lab">
      <div className="cp-controls">
        <label>
          <span>CP length <strong><MathExpr tex={`N_{\\mathrm{CP}}=${cpLength}`} /></strong></span>
          <input type="range" min="0" max="8" step="1" value={cpLength}
            onChange={(e) => setCpLength(Number(e.target.value))} />
        </label>
        <label>
          <span>Channel memory <strong><MathExpr tex={`D=${channelDelay}`} /></strong></span>
      ...
      <div className="cp-readout">
        <div><small>USEFUL</small><strong><MathExpr tex="N=16" /></strong><span>samples after CP removal</span></div>
        <div><small>CP</small><strong><MathExpr tex={`N_{\\mathrm{CP}}=${cpLength}`} /></strong><span>copy from end of symbol</span></div>
        <div><small>CHANNEL MEMORY</small><strong><MathExpr tex={`D=${channelDelay}`} /></strong><span>simplified max delay</span></div>
      ...
      <div className="cp-visual">
        ...
        <div className="cp-copy-note">
          <span>CP copy</span>
          <b>← last <MathExpr tex={`${cpLength}`} /> samples of useful symbol</b>
        </div>

        <div className="cp-channel-memory">
          <span>Echo/channel memory extends roughly</span>
          <strong><MathExpr tex={`${channelDelay}`} /> samples</strong>
          <i style={{ width: `${Math.min(100, channelDelay / 10 * 100)}%` }}></i>
        </div>

        <div className={`cp-fft-window ${sufficient ? 'safe' : 'unsafe'}`}>
          <strong>Receiver removes CP → FFT uses <MathExpr tex="N=16" /> useful samples</strong>
          <p>
            {sufficient
